...
新增表单组件建好后，根据 `submit` 回调逻辑，页面会跳转到主页展示出已经添加的所有图片。因此需要新增主页组件 `src/components/HomePage.vue`：
...
注意第 40 行代码，这里用到了 `this.$router.push()` 方法来跳转到某个路由。参数是一个包含 `path` 属性的 `JSON` 对象，`path` 的值，为目标 `URL` 与携带参数 `id` 的组合形式（`REST` 风格）。

另外，HTML 标签内是不需要插值符号 `{{}}` 的，仅在需要显示在页面的位置需要使用。

CSS 样式部分采用了 `flex` 盒模型，将 `div` 以表格形式渲染到主页上。
